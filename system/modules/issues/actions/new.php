...
if ($_POST['create']) {
	...
	if (!errors()) {
		...
		$issueid = $_ENV['dbi']->insert('issues',$insert,'issues_issueid_seq');
		if (!is_null($issueid)) {
...
if (empty($_GET['gid'])) {
	if ($_SESSION['group_count'] > 1) {
		Module::template('issues','choose_new.tpl');
	} else {
	...
	if (Permission::check('create_issues',$_GET['gid'])) {
		if (!empty($_GET['icopy'])) {
			if (can_view_issue($_GET['icopy'])) {
				$sql = "SELECT summary,problem FROM issues WHERE issueid='".$_GET['icopy']."'";
				list($summary,$problem) = $_ENV['dbi']->fetch_row($sql);
				if (!empty($summary)) {
		...
		$_ENV['tpl']->assign('products',group_products($_GET['gid']));
		$_ENV['tpl']->assign('members',group_members($_GET['gid']));
		Module::template('issues','new.tpl');
	} else {
		redirect();
	}
}
